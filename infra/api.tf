...
# Create an empty secret "slot" (no value here)
resource "aws_secretsmanager_secret" "mesh_api" {
    name = "mesh-aws/mesh-api"
}
...
# IAM policy: allow the Lambda to read that secret
resource "aws_iam_policy" "lambda_read_mesh_secret" {
    name   = "mesh-aws-lambda-read-mesh-secret"
    policy = jsonencode({
        Version = "2012-10-17",
        Statement = [{
            Effect: "Allow",
            Action: ["secretsmanager:GetSecretValue"],
            Resource: aws_secretsmanager_secret.mesh_api.arn
        }]
    })
}

resource "aws_iam_role_policy_attachment" "lambda_read_mesh_secret_attach" {
    role       = aws_iam_role.lambda_exec.name
    policy_arn = aws_iam_policy.lambda_read_mesh_secret.arn
}
...
resource "aws_lambda_function" "api" {
    ...
    environment {
        variables = {
            SECRET_NAME = aws_secretsmanager_secret.mesh_api.name
        }
    }
}
